...
#define BIN_PATH "./geekNameFormatter"
...
Test(geek_name_formatter, geek1)
{
    char out[256];
    char *argv[] = {BIN_PATH, "santa Tomato!KETCHUP/", NULL};

    run_bin(argv, out, sizeof(out));
    cr_assert_str_eq(out, "S4Nt4 T0M4T0!k3tCh2p/ \n");
}

Test(geek_name_formatter, geek2)
{
    char out[256];
