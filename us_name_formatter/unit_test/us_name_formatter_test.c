...
#define BIN_PATH "./USNameFormatter"
...
Test(us_name_formatter, us1)
{
    char out[256];
    char *argv[] = {BIN_PATH, "pJ HARVEY", NULL};

    run_bin(argv, out, sizeof(out));
    cr_assert_str_eq(out, "PJ Harvey\n");
}

Test(us_name_formatter, us2)
{
    char out[256];
    char *argv[] = {BIN_PATH, "MiCkAeL j FoX", NULL};

    run_bin(argv, out, sizeof(out));
    cr_assert_str_eq(out, "Mickael J. Fox\n");
}

Test(us_name_formatter, us3)
{
    char out[256];
